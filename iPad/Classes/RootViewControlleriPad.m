...
- (void)viewDidLoad {
	...
	//[[self view] setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"iPadBackground.png"]]];
...
- (void)didRotate:(NSNotification *)notification { 
	...
	//[adView setFrame:CGRectMake(0, 0, self.view.bounds.size.width, 50)];
...
- (IBAction)newGame:(id)sender {
	[pileDeal setImage:[UIImage imageNamed:@"back-blue-150-2.png"]];
...
- (BOOL)addCardToPile:(NSDictionary *)aCard atIndex:(int)aIndex {
	...
	NSString *aImageName = [aCard objectForKey:@"imagelg"];
...
- (void)dispatchFirstTouchAtPoint:(CGPoint)touchPoint forEvent:(UIEvent *)event {
	if (CGRectContainsPoint([pileDeal frame], touchPoint)) {		
		...
		draggingImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[pileDealCard objectForKey:@"imagelg"]]];
...
- (UIImageView *)getPileDealCardImage {
	NSDictionary *pileDealCard = [[self shuffledDeck] objectAtIndex:[[self cardCount] intValue]];
	return [[UIImageView alloc] initWithImage:[UIImage imageNamed:[pileDealCard objectForKey:@"imagelg"]]];
}

- (UIImageView *)getCardBackImage {
	return [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"back-blue-150-2.png"]];
}
...
- (BOOL)dealCard:(NSDictionary *)aCurrentPileDealCard onCard:(NSDictionary *)aMatchedCard {
	...
	NSString *aImageName = [aCurrentPileDealCard objectForKey:@"imagelg"];
...
- (IBAction)alertWelcome {
	...
	if([self isGameCenterEnabled]) {
		GameCenterViewControlleriPad *controller = [[GameCenterViewControlleriPad alloc] initWithNibName:@"GameCenterViewControlleriPad" bundle:nil];
		...
		NSString *title = @"Double Match 7.1.0";
...
- (void)notAWinner {
	...
	NSString *title = @"I'm Sorry";
	NSString *mess = @"There are no more pairs to match.";
	// open a alert with an OK and cancel button
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title message:mess
												   delegate:self 
										  cancelButtonTitle:nil 
...
- (void)congrats {
	...
	NSString *mess = @"Congratulations, YOU DID IT !!!";
	// open a alert with an OK and cancel button
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title message:mess
												   delegate:self 
										  cancelButtonTitle:nil 
										  otherButtonTitles:@"You Rock !", nil];
	[alert show];
	[self addWin];
}
...
#pragma mark -
#pragma mark iPad Related Methods
	...
	if([self isGameCenterEnabled]) {
		if(error == NULL) {
			...
			if([leaderBoard.scores count] >0) {
				...
				[[self gameCenterManager] mapPlayerIDtoPlayer:allTime.player.playerID];
			}
		} else {
